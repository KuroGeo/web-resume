...
let versions=[],doc=null,group='',mode='form',changes={},collectionChanges={},sourceDirty=false,busy=false,page=1;
...
async function choose(version){if(busy||version===doc?.version)return;if(dirty()&&!confirm('当前修改尚未保存。放弃修改并切换版本？'))return;setBusy(true,'正在载入版本…');$('#error').hidden=true;try{const next=await api('document?version='+encodeURIComponent(version));doc=next;changes={};collectionChanges={};sourceDirty=false;page=1;group='';fill();}catch(e){error(e.message);}finally{setBusy(false,doc?'已载入本机内容':'请重新载入');}}
function proof(){const p=doc?.preview;$('#empty-preview').hidden=!!p;$('#page').hidden=!p;$('#download').hidden=!p;if(!p){$('#page-count').textContent='— / —';$('#preview-state').textContent='尚未生成';$('#prev').disabled=$('#next').disabled=true;return;}page=Math.min(page,p.pages);$('#page-image').src=artifact('page-'+page+'.png');$('#page-image').alt=doc.name+'的简历 PDF，第 '+page+' 页';$('#page-count').textContent=page+' / '+p.pages;$('#page-caption').textContent='第 '+page+' 页，共 '+p.pages+' 页';$('#prev').disabled=page<=1;$('#next').disabled=page>=p.pages;$('#download').href=artifact('document.pdf');$('#download').download=doc.version+'.pdf';status();}
...
async function action(render){if(busy||!doc)return;$('#error').hidden=true;setBusy(true,render?'正在保存并排版…':'正在保存…');try{await save();if(render){$('#rendering').hidden=false;const generated=await api('render',{version:doc.version,revision:doc.revision});doc.preview=generated;page=1;proof();}status();}catch(e){error(e.message);}finally{$('#rendering').hidden=true;setBusy(false);}}
